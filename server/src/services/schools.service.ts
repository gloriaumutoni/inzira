...
export const update = async (id: string, data: { name?: string; district?: string; isActive?: boolean }) => {
...
export const assignCareerGuide = async (schoolId: string, email: string) => {
  ...
  if (user.role !== 'CAREER_GUIDE') throw new Error('This user is not a career guide')
