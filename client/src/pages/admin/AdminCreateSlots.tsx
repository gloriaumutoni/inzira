...
const weeksToEndDate = (weeks: number | null): string | null => {
...
const AdminCreateSlots = () => {
  ...
  const handleAdd = async () => {
    ...
    try {
      await addSlotMutation.mutateAsync({
        ...
        endDate: weeksToEndDate(repeatWeeks),
      })
  ...
  const saveEdit = async () => {
    ...
    const { dayOfWeek: d, startHour: sh, startMinute: sm, endHour: eh, endMinute: em, meetLink: ml, repeatWeeks: rw } = editState
                ...
                <div key={slot.id}>
                  {editingId === slot.id && editState ? (
                    <div className="border border-accent/30 rounded-lg p-3 bg-accent/5 space-y-3">
                      ...
                      <div>
                        <label htmlFor={`edit-weeks-${slot.id}`} className="block text-xs font-medium text-muted mb-1">Repeat for</label>
                        <select
                          id={`edit-weeks-${slot.id}`}
                          value={editState.repeatWeeks ?? ''}
                          onChange={(e) => setEditState({ ...editState, repeatWeeks: e.target.value ? Number(e.target.value) : null })}
                          className="w-full border border-border rounded-lg px-2 py-1.5 text-xs text-primary bg-surface"
                        >
                    ...
                    <div className="flex items-center justify-between gap-2 flex-wrap py-2 px-3 border border-border rounded-lg hover:border-border/70">
                      <div className="flex flex-col">
                        ...
                        <span className="text-xs text-subtle">{formatEndDate(slot.endDate)}</span>
                      </div>
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <select
                ...
                className="w-full border border-border rounded-lg px-3 py-2 text-sm text-primary bg-surface"
              >
                <option value="">No end date (recurs indefinitely)</option>
                {WEEKS_OPTIONS.map((w) => (
                  <option key={w} value={w}>{w} weeks</option>
                ))}
              </select>
